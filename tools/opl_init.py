...
def validate_number(val: str, lo: int = 1, hi: int = 60) -> bool:
    try:
        n = int(val)
        return lo <= n <= hi
    except ValueError:
...
def generate_notice(args: argparse.Namespace) -> str:
    lines = ["# NOTICE file for Open-Pact License v1.3.1", "# Generated by opl_init.py", "", f"OPL Version: {args.version}", f"Maintainer: {args.maintainer}", f"Governing Jurisdiction: {args.jurisdiction}", f"Standard Terms URL: {args.terms_url}", f"OPL-AI: {args.opl_ai}"]
...
def interactive_mode() -> argparse.Namespace:
    print(BANNER)
    print("  This wizard creates a NOTICE file for your OPL-1.3.1 project.\n")
    ...
    args.opl_ai = "opted in. AI training is restricted under the OPL-AI addendum (v1.3.1)." if ai.lower().startswith("in") else "opted out."
...
def main() -> None:
    ...
    if cli.non_interactive:
        if not all([cli.maintainer, cli.jurisdiction, cli.terms_url]):
            ...
            sys.exit(1)
        args = argparse.Namespace(version="1.3.1", maintainer=cli.maintainer, jurisdiction=cli.jurisdiction, terms_url=cli.terms_url, opl_ai="opted in. AI training is restricted under the OPL-AI addendum (v1.3.1)." if cli.opl_ai == "in" else "opted out.", abandonment=cli.abandonment, trademark=cli.trademark)
    else:
    ...
    print("    2. Add SPDX-License-Identifier: OPL-1.3.1 to each source file (use opl_spdx_inject.py).")
