...
def load_params_from_args(cli: argparse.Namespace) -> dict:
    ...
    p["jurisdiction_value"] = cli.jurisdiction_value or "Berlin, Germany"
    return p
...
def check_hard_blocks(params: dict, resolved: dict) -> list[str]:
    warnings = []
    is_fair_source = True
    for slot_key, opt in resolved.items():
...
def build_schedule(params: dict) -> str:
    ...
    for slot_key, title in slot_titles.items():
        ...
        text = text.replace("N months", f"{params.get('dosp_months_value', 36)} months") \
                    ...
                    .replace("the vetted jurisdiction", f"**{params.get('jurisdiction_value', 'Berlin, Germany')}**") \
                    .replace("the custom jurisdiction", f"**{params.get('jurisdiction_value', 'Berlin, Germany')}**")
...
def generate_notice(params: dict) -> str:
    lines = ["# NOTICE file for Custom OPL (based on OPL-1.4)", "# Generated by custom_opl.py", "",
             f"OPL Version: 1.4 (Custom OPL variant)",
             f"Maintainer: {params.get('maintainer', 'Unspecified')}",
             f"Governing Jurisdiction: {params.get('jurisdiction_value', 'Berlin, Germany')}",
...
def main() -> None:
    ...
    ap.add_argument("--jurisdiction-value", default="Berlin, Germany")
